...
class ProductEnrichment(BaseModel):
    ...
    subcategories: List[str] = Field(
        description="A list of specific subcategories. For Clothing, examples could be 'Dress', 'Sci-Fi Armor', 'Full Outfit'. For Character, 'Female', 'Male', 'Creature'."
    )

    styles: List[str] = Field(
        description="A list of descriptive styles or genres. Examples: 'Sci-Fi', 'Fantasy', 'Cyberpunk', 'Gothic', 'Contemporary', 'Historical', 'Steampunk', 'Photo-realistic'."
    )

    inferred_tags: List[str] = Field(
        description="A list of 5-10 additional useful, specific keywords inferred from the description that are not styles or categories. Examples: 'leather', 'weapon', 'game-ready', 'futuristic', 'high-detail'."
    )
...
class LocalTransformerEnricher:
    ...
    def __init__(self, model_name: str):
        ...
        print(
            "Loading quantized model (this may take a moment and download on first run)..."
        )
    ...
    def enrich(self, product_info: dict) -> ProductEnrichment:
        ...
        **Product Information:**
        ...
        structured_output = self.model(
            input_text, ProductEnrichment, max_new_tokens=1024
        )
    ...
    try:
        if LLM_PROVIDER == "local":
            model_name = os.getenv(
                "LOCAL_TRANSFORMER_MODEL_NAME", "google/gemma-2-9b-it"
            )
            enricher = LocalTransformerEnricher(model_name=model_name)
        else:
            raise NotImplementedError(
                f"Provider '{LLM_PROVIDER}' is not implemented yet."
            )
