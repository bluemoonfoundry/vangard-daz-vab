...
def build_where_clause(
    ...
    def create_or_condition(field_name: str, values: List[str]):
        ...
        # For fields stored as JSON strings of lists, we use substring matching ($contains)
            ...
            for value in values:
                print (f'+++++ Test field_name: {field_name}, values: {value}, type: {type(value)}')
...
def get_db_stats():
    ...
    tag_counter, artist_counter, figure_counter, category_counter = Counter(), Counter(), Counter(), Counter()

    # Find the Last Update Date
    last_update_dates = [meta.get('last_updated') for meta in all_metadatas if meta.get('last_updated')]
