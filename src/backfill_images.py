...
def backfill_image_urls():
    ...
    print(f"\nReading image URL data from: {product_file}")
    try:
        ...
        print(
            f"Error: Could not process '{JSON_SOURCE_FILE}'. Ensure it's a valid JSON array of objects with 'sku' and 'image_url' keys.",
            file=sys.stderr,
        )
