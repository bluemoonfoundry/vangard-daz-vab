...
class BaseAssetSpider(scrapy.Spider):
    ...
    async def start(self):
        for req in self.start_requests(): yield req
    ...
    def parse_product(self, response):
        ...
        if figure_data_from_meta and isinstance(figure_data_from_meta, list):
            pre_existing_figures = [fig.get('category') for fig in figure_data_from_meta if fig.get('category')]
        # --- END CHANGE ---
        ...
        for field in item.fields:
            if field not in ['url', 'store', 'sku', 'mature', 'category'] and hasattr(self, f"extract_{field}"):
        ...
        if scraped_sku and scraped_sku != item['sku']:
            self.logger.warning(f"SKU mismatch for URL {response.url}. Passed in: {item['sku']}, Scraped: {scraped_sku}")
            
        yield item
        
    def _execute_selector(self, response, selector, get_all=False):
        if not selector: return [] if get_all else None
    ...
    def extract_name(self, response): return self._execute_selector(response, self.selectors['name'])
    def extract_sku(self, response): return self._execute_selector(response, self.selectors['sku'])
    def extract_artist(self, response): return self._execute_selector(response, self.selectors['artist'], get_all=True)
    def extract_price(self, response): return self._execute_selector(response, self.selectors['price'])
    def extract_tags(self, response): return self._execute_selector(response, self.selectors['tags'], get_all=True)
    def extract_formats(self, response): return self._execute_selector(response, self.selectors['formats'], get_all=True)
...
#    def extract_textures_info(self, response): return self._execute_selector(response, self.selectors['textures_info'])
    def extract_required_products(self, response): return self._execute_selector(response, self.selectors['required_products'], get_all=True)
    def extract_compatible_figures(self, response): return self._execute_selector(response, self.selectors['compatible_figures'], get_all=True)
    def extract_compatible_software(self, response): return self._execute_selector(response, self.selectors['compatible_software'], get_all=True)
    def extract_description(self, response):
        parts = self._execute_selector(response, self.selectors['description'], get_all=True)
