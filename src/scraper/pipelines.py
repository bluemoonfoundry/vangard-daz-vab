...
class AssetProcessingPipeline:
    ...
    def process_item(self, item, spider):
        # --- DEBUG LOGGING ---
        spider.logger.info(f"[AssetProcessingPipeline] Received item with SKU: {item.get('sku')}")
        # --- END DEBUG ---
        ...
        for field in ['artist', 'tags', 'formats', 'required_products', 
                      'compatible_figures', 'compatible_software']:
            if item.get(field):
        ...
        if compat_parts:
            parts.append(f"This asset is designed so that it is {' and '.join(compat_parts)}.")
