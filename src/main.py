...
from fetch_daz_data import main as fetch_daz_data, product_file
...
from query_utils import search, get_db_stats
            ...
            if 'url' not in product or not product['url']:
                if product['store_id'] == 1: # DAZ Store
                    ...
                    if content is not None: 
                        ...
                        image_url=image_root_url[image_root_url.rfind('https://gcdn'):]
                        ...
                        print(f"Info: Computed DAZ Store URL for '{product['title']}' as '{product['url']}' with image_url '{image_url}'.")
                else:
                    print(f"Warning: No URL computable for '{product['title']}' (Store ID: {product['store_id']}).")
...
def scrape_command(args):
    ...
    if args.update:
        ...
        products_to_scrape = [
            ...
            for p in products if p.get('url') and p.get('sku')
        ]
    
    if args.limit and args.limit > 0:
        print(f"Applying limit: processing only the first {args.limit} of {len(products_to_scrape)} products.")
...
def rebuild_command(args):
    ...
    confirm = input("WARNING: This will delete and rebuild the entire ChromaDB collection. "
...
def load_command(args):
    ...
    checkpoint = get_checkpoint()
    load_sqlite_to_chroma(checkpoint)
    set_checkpoint()
...
def main():
    parser = argparse.ArgumentParser(description="Visual Asset Browser Data Pipeline CLI")
    parser.add_argument("--product-file", type=str, default="./products.json", help="Path to JSON file containing product data.")    
    subparsers = parser.add_subparsers(dest="command", required=True)

    # Define commands
    parsers = {
        "fetch": subparsers.add_parser("fetch", help="Calls external library to create/update products.json."),
        "scrape": subparsers.add_parser("scrape", help="Scrape products and save to SQLite."),
        "enrich": subparsers.add_parser("enrich", help="Use an LLM to enrich product data."),
        "rebuild": subparsers.add_parser("rebuild", help="Nuke and rebuild the entire ChromaDB from SQLite."),
        "load": subparsers.add_parser("load", help="Load new data from SQLite into ChromaDB."),
        "query": subparsers.add_parser("query", help="Query the ChromaDB vector store."),
        "stats": subparsers.add_parser("stats", help="Display stats about the ChromaDB collection."),
        "server": subparsers.add_parser("server", help="Run the FastAPI web server."),
        "openproduct": subparsers.add_parser("openproduct", help="Open a naed DAZ product in DAZ Studio's Content Library Pane")
    }

    # Add arguments
    parsers["scrape"].add_argument("--update", action="store_true", help="Only scrape products newer than the last checkpoint.")
    parsers["scrape"].add_argument("--limit", type=int, default=None, help="Limit the number of URLs to process from the list.")    

    parsers["query"].add_argument("prompt", help="The search prompt.")
    parsers["query"].add_argument("--tags", nargs='*', help="List of tags to pre-filter results.")
    parsers["query"].add_argument("--limit", type=int, default=5, help="Max number of results.")
    parsers["query"].add_argument("--score", type=float, default=1.0, help="Maximum distance (lower is better).")
    parsers["query"].add_argument("--sort-by", default="relevance", help="Field to sort by ('relevance', 'name').")
    parsers["query"].add_argument("--sort-order", choices=['ascending', 'descending'], default='descending')
    ...
    parsers["server"].add_argument("--demo", action="store_true", help="Run server in demo mode.")

    parsers["openproduct"].add_argument("--product", help="The name of the product to open in DAZ Studio.")
